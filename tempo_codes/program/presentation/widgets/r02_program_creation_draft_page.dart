...
class _R02ProgramCreationDraftPageState
  ...
  void _handleQuantityTap() async {
    ...
    print('🔍 Program state: ${programState.runtimeType}');

    programState.maybeWhen(
      loaded: (programSettings, contractorRoads) {
        print('📦 Program settings count: ${programSettings?.length ?? 0}');
        ...
        try {
          ...
          currentSetting = programSettings.isNotEmpty
              ? programSettings.first
              : null;
        }
        ...
        if (currentSetting == null) {
          CustomSnackBar.show(
            context,
            'No program settings available for this work scope',
            type: SnackBarType.warning,
          );
          return;
        }

        print('📋 Found setting: ${currentSetting.uid}');
        print(
          '   Quantity types: ${currentSetting.quantityTypes?.length ?? 0}',
        );
        ...
        // Debug: Print quantity types
        currentSetting.quantityTypes!.forEach((qt) {
          print('   - ${qt.name} (${qt.uid})');
        });
      ...
      orElse: () {
        print('❌ Program state is not loaded');
        CustomSnackBar.show(
          context,
