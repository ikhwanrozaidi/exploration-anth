...
abstract class DailyReportLocalDataSource {
  ...
  Future<({CreateDailyReportModel model, String companyUID})?> getUnsyncedReportData(
    String uid,
  );
...
class DailyReportLocalDataSourceImpl implements DailyReportLocalDataSource {
  ...
  Future<DailyReportResponse> createDailyReportLocal(
    ...
    try {
      ...
      final company = await (_database.select(_database.companies)
            ..where((tbl) => tbl.uid.equals(companyUID)))
          .getSingleOrNull();
  ...
  @override
  Future<({CreateDailyReportModel model, String companyUID})?> getUnsyncedReportData(String uid) async {
    try {
      final record = await (_database.select(_database.dailyReports)
            ..where((tbl) => tbl.uid.equals(uid) & tbl.isSynced.equals(false)))
          .getSingleOrNull();
      ...
      final company = await (_database.select(_database.companies)
            ..where((tbl) => tbl.id.equals(record.companyID)))
          .getSingleOrNull();
