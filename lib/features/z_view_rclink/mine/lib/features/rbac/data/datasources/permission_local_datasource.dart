...
abstract class PermissionLocalDataSource {
  Future<Role?> getCachedRole();
  Future<void> cacheRole(Role role);
...
class PermissionLocalDataSourceImpl implements PermissionLocalDataSource {
  ...
  @override
  Future<Role?> getCachedRole() async {
    ...
    final permissionRecords = await (_database.select(_database.permissions)
      ..where((p) => p.roleID.equals(roleRecord.id)))
      .get();
    
    // Convert to domain entities
    final permissions = permissionRecords.map((p) => Permission(
      id: p.id,
      uid: p.uid,
      code: p.code,
      name: p.name,
  ...
  @override
  Future<void> cacheRole(Role role) async {
    ...
    final insertedRole = await (_database.select(_database.roles)
      ..where((r) => r.uid.equals(role.uid)))
      .getSingle();
    
    // Insert permissions
    for (final permission in role.permissions) {
      await _database.into(_database.permissions).insert(
