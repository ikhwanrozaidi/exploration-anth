...
class SubmitDailyReportUseCase {
  ...
  Future<Either<Failure, DailyReport>> call({
    ...
    try {
      ...
      print(
        '📷 Condition snapshots keys: ${reportData.selections.conditionSnapshots.keys.toList()}',
      );
      print(
        '📷 Worker images count: ${reportData.selections.workerImages.length}',
      );

      final images = _extractImagesFromSelections(reportData.selections);

      print(
        '📷 Extracted images: ${images?.keys.map((k) => '${k.value}: ${images[k]?.length}').join(', ') ?? 'none'}',
      );
